...
\subsection{Length function}
    ...
    Apply \cref{lem:length_of_extension} to \(A = \calO_{X,V}\), where \(X\) is a variety and \(V\) is a subvariety of codimension \(1\).
    ...
    Here \(\calO_{X,X_j}\) is an Artinian local ring, so \(\call(\calO_{X,X_j}) < \infty\).
...
\subsection{Flat pullbacks}
    ...
    In this note, by a \emph{flat morphism}, we always mean a flat morphism of constants relative dimension.
    ...
    \begin{proof}
        ...
        The coefficient of \([V]\) in \([Z]\) (resp. $[W]$ in $[f^{-1}(V)]$) is \(\call(\calO_{Z,V})\) (resp. $\call(\calO_{f^{-1}(V),W})$).
        Hence the coefficient of \([W]\) in \(f^*[Z]\) is $\call(\calO_{Z,V}) \cdot \call(\calO_{f^{-1}(V),W})$.
        Note that 
        \[ \calO_{f^{-1}(V),W} = \calO_{f^{-1}(Z),W}/(\frakm_{Z,V}\calO_{f^{-1}(Z),W}). \]
        On the other hand, the coefficient of \([W]\) is \([f^{-1}(Z)]\) is $\call(\calO_{f^{-1}(Z),W})$.
        Hence the proof reduced to show 
        \[ \call(\calO_{Z,V}) \cdot \call(\calO_{f^{-1}(Z),W}/(\frakm_{Z,V}\calO_{f^{-1}(Z),W})) = \call(\calO_{f^{-1}(Z),W}). \]
        Here $\calO_{Z,V} \to \calO_{f^{-1}(Z),W}$ is a flat local homomorphism of Artinian local rings, so the above formula follows from \cite[A.4.1]{Fulton13Intersection}.
    \end{proof}
    ...
    \begin{proposition}
        ...
        Then for every \(\alpha \in Z_\bullet(X)\), we have
        \[ g^*f_*\alpha = f'_*g'^*\alpha \in Z_\bullet(Y'). \]
    \end{proposition}
    \begin{proof}
        \hspace{1em}
        \begin{case}
    ...
    \end{theorem}
    \begin{proof}
        By \cref{prop1}, may assume \(\alpha = [g^{-1}(0)] - [g^{-1}(\infty)]\) for \(V\) an \(i+1\)-dimensional subvariety of \(\bbP^1 \times Y\) such that the projection \(g: V \to \bbP^1\) is dominant.
        In particular, \(g\) is flat.
        We have a diagram:
        \[ \begin{tikzcd}
            W \arrow[r, hook] \arrow[d] \arrow[ddr,bend right=90,"h"'] & \bbP^1\times X \arrow[d, "\id\times f"] \arrow[r, "q"] & X \arrow[d, "f"] \\
                        V \arrow[r,hook] \arrow[dr,"g"]  & \bbP^1 \times Y \arrow[d] \arrow[r, "p"] & Y \\
                            & \bbP^1 &
        \end{tikzcd} \]
        Then 
        \begin{align*}
            f^*\alpha & = f^*p_*([g^{-1}(0)] - [g^{-1}(\infty)]) \\
            & = q_*(\id \times f)^*([g^{-1}(0)] - [g^{-1}(\infty)]) \\
            & = q_*([h^{-1}(0)] - [h^{-1}(\infty)]).
        \end{align*}
        Let \(W_1, \ldots, W_t\) be the irreducible components of \(W\), \(h_j = h|_{W_j}\).
        Suppose that \([W] = \sum_{j=1}^t m_j [W_j]\).
        By \cref{lem:restriction_of_cartier_divisor_to_irreducible_components}, we have \([h^{-1}(0)] = \sum_{j=1}^t m_j [h_j^{-1}(0)]\) and \([h^{-1}(\infty)] = \sum_{j=1}^t m_j [h_j^{-1}(\infty)]\).
        Hence
        \begin{align*}
            f^*\alpha =  m_j q_*\left(\sum_{j=1}^t[h_j^{-1}(0)] - [h_j^{-1}(\infty)]\right) = \sum_{j=1}^t m_j q_*([h_j^{-1}(0)] - [h_j^{-1}(\infty)]) \sim 0.
        \end{align*}
        We are done.
        % \Yang{}
    \end{proof}
...
\subsection{Localization exact sequence}
    ...
    \begin{proof}[Ideal of Proof]
        ...
        Passing to the quotient, we get a complex
        \[ \CH_i(Y) \xrightarrow{i_{Y*}} \CH_i(X) \xrightarrow{j_U^*} \CH_i(U) \to 0. \]
        We only need to show that if \(\alpha \in Z_i(X)\) and \(j_U^*\alpha \sim 0\) on \(U\), then \(\alpha \in i_{Y*}Z_i(Y) + \sim\) on \(X\).
        We can assume that \(j_U^*\alpha = \divisor(f)\) for some \(f \in \fkk(V)^\times\) with \(V \subset U\) a subvariety of dimension \(i+1\).
        Consider \(f\) as a rational function on \(\overline{V}\).
        Then \(\alpha - \divisor(f) = i_{Y*}\beta \in Z_i(X)\) for some \(\beta \in Z_i(Y)\) by the exact sequence on the level of cycles.
        Hence \(\alpha \sim i_{Y*}\beta\) on \(X\) and we are done.
    \end{proof}

    \begin{example}
        Let \(L_i \subset \bbP^n\) be a linear subspace of dimension \(i\) for \(i = 0, \ldots, n\).
        Then $\CH_i(\bbP^n)$ is generated by \([L_i]\) for \(i = 0, \ldots, n\).
        Indeed, we have an exact sequence
        \[ \CH_i(L_{n-1}) \xrightarrow{\iota_*} \CH_i(\bbP^n) \xrightarrow{j^*} \CH_i(\bbA^n) \to 0. \]
        The assertion follows from \(\CH_i(\bbA^n) = 0\) for \(i < n\) and \(\CH_n(L_{n-1}) = 0\) for \(i=n\).
        We will see later that \(\CH_i(\bbP^n) = \bbZ\cdot [L_i]\).
    \end{example}
