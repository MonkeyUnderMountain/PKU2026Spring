...
\subsection{Chern classes}
  ...
  \begin{definition}
    ...
    \[ c_t(E) \coloneqq \frac{1}{s_t(E)} \in R_X [[t]] \]
  ...
  \end{align*}
  % \Yang{check}
  The homomorphisms $s_j(E), c_j(E): \CH_\bullet(X) \to \CH_{\bullet - j}(X)$ shift the degree by $j$.
  ...
  \begin{proof}
    ...
    Denote by $j: Y \hookrightarrow X$ the closed embedding.
    Restricting the above short exact sequence to $Y$, we see that $s|_Y$ is contained in $E^1|_Y$ and is nowhere vanishing.
    By induction on the rank of $E$, we have $\prod_{u=2}^r c_1(L_u|_Y) = 0$ in $R_Y$.
    Consider the pseudo-divisor $D = (L_1, Y, s_1)$ on $X$.
    Then for any $\alpha \in \CH_\bullet(X)$, we have \Yang{}
    \[ c_1(L_1) \prod_{u=2}^r c_1(L_u) \cdot \alpha = j_* \left( D \cdot \prod_{u=2}^r c_1(L_u) \cdot \alpha \right) = j_* \left(\prod_{u=2}^r c_1(L_u|_Y) \cdot D \cdot \alpha \right) = 0. \]
    Here the first is by \Yang{} and the second is by \Yang{}.
    Hence

    \[ \prod_{u=1}^r c_1(L_u) = j_* \left( c_1(L_1|_Y) \prod_{u=2}^r c_1(L_u|_Y) \right) = 0. \]
  ...
  \begin{proof}
    ...
    Then by \cref{eq:product_of_twisted_chern_classes_vanishes}, we have
